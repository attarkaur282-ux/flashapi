...
<head>
    ...
    <style>
        ...
        .btn { padding: 10px 20px; border: none; border-radius: 10px; cursor: pointer; font-weight: bold; margin: 5px; text-decoration: none; display: inline-block; }
        ...
        .flex .btn { flex: 1; min-width: 80px; text-align: center; }
        ...
        .badge-ban { background: #ff9800; color: black; }
    </style>
</head>
<body>
...
<div class="card">
    <h3>📊 Status</h3>
    <p>Flash: <span class="badge badge-''' + flash_status + '''">''' + flash_status.upper() + '''</span></p>
    <p>API: <span class="badge ''' + ('badge-ban' if api_banned else 'badge-on') + '''">''' + ('BANNED' if api_banned else 'ACTIVE') + '''</span></p>
...
def api():
    ...
    if user_ip not in users:
        users[user_ip] = {"count": 0, "last_action": "first"}
    ...
    if api_banned:
        if ban_type == "temp" and ban_until:
            if datetime.datetime.now() > datetime.datetime.fromisoformat(ban_until):
                ...
                ban_type = "none"
                ban_until = None
                add_log("⏰ Temp ban expired")
            else:
                users[user_ip]["last_action"] = "temp_banned"
                return jsonify({
                    "error": f"API temp banned until {ban_until}",
                    "owner": OWNER
                }), 403
        else:
            users[user_ip]["last_action"] = "banned"
            return jsonify({"error": "API is banned", "owner": OWNER}), 403
    ...
    if action == 'on':
        flash_status = 'on'
        add_log(f"Flash ON via API")
        return jsonify({"success": True, "flash": "ON", "owner": OWNER})
    elif action == 'off':
        flash_status = 'off'
        add_log(f"Flash OFF via API")
...
def admin_on():
    ...
    return '<script>alert("Flash ON!"); window.location="/admin";</script>'
...
def admin_off():
    ...
    return '<script>alert("Flash OFF!"); window.location="/admin";</script>'
...
def admin_ban():
    ...
    ban_type = "permanent"
    ban_until = None
    add_log("API PERMANENTLY BANNED")
    return '<script>alert("API BANNED!"); window.location="/admin";</script>'
...
def admin_tempban(minutes):
    ...
    add_log(f"API TEMP BANNED {minutes}min")
    return f'<script>alert("Temp banned {minutes} min!"); window.location="/admin";</script>'
...
def admin_clearlogs():
    ...
    return '<script>alert("Logs cleared!"); window.location="/admin";</script>'
